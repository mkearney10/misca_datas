...
while n < df2.shape[0]:
    ...
    while l < 6:
        ...
        if (cat_cnts[catid] >= 6 and cat_cnts[catid] < 11 and df2.loc[n,'Rank_'+str(l+1)] == 1) or (cat_cnts[catid] >=11 and df2.loc[n,'Rank_'+str(l+1)] <=3):
            tmp_upgrd += 1
        if df2.loc[n, 'RankPct_'+str(l+1)] <= mand_upgrd:
            tmp_upgrd_2 += 1
        if df2.loc[n, 'RankPct_'+str(l+1)] <= sugg_upgrd:
            tmp_upgrd_3 += 1
    ...
    if df2.loc[n,'Grade_25'] > 12:
        ...
        if (mand_upgrd == 0 and tmp_upgrd >= 2) or (mand_upgrd > 0 and tmp_upgrd_2 >= upgrd_thresh):
            if grade in [5] and catid in [601]:
                ...
                df2.loc[n,'CatID_25_Upgrd_1'] = df2.loc[n,'CatID_25_dflt']
        elif (sugg_upgrd > 0 and tmp_upgrd_3 >= upgrd_thresh):
            if grade in [5] and catid in [601]:
